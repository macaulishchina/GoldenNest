...
class Role(Base):
    ...
    # ---- AI 对话配置 ----
    role_prompt = Column(Text, nullable=False)                 # 角色定义 (谁在说话)
    strategy_prompt = Column(Text, nullable=False)             # 对话策略 (如何对话)
    tool_strategy_prompt = Column(Text, default="")            # 工具使用策略
    finalization_prompt = Column(Text, default="")             # 定稿提示 (何时/如何定稿)
    output_generation_prompt = Column(Text, default="")        # 产出物生成 prompt 模板
                                                               # 模板变量: {discussion_summary}
    ...
    # 格式: {
    #   "project_noun": "需求",           # 项目称呼
    #   "create_title": "🆕 新建需求",   # 创建对话框标题
    #   "create_placeholder": "简明描述需求目标",
    #   "description_placeholder": "详细描述需求背景和期望效果...",
    #   "output_noun": "需求规格书",      # 产出文档名称
    #   "output_tab_label": "📋 设计稿", # 产出文档 tab 标签
    #   "finalize_action": "敲定方案",    # 定稿动作名称
    # }
...
### 对话策略
1. **主动提问** — 用户描述需求后，立即用 `ask_user` 工具提出 3-5 个关键问题来澄清需求。
2. **聚焦「做什么」** — 讨论应围绕：用户故事、交互流程、业务规则、边界条件、优先级。
3. **连续提问** — 如果一轮回答后仍有不明确的地方，继续用 `ask_user` 追问。
4. **总结确认** — 每轮问答后，简要总结你对需求的理解，让用户确认或纠正。
5. **循循善诱** — 帮助用户发现他们没想到的需求场景。
...
请直接输出诊断书内容（不需要代码块包裹）:""",
    "stages": [
        ...
        {"key": "planned", "label": "确认", "status": "planned"},
...
**编辑视图**（Modal 或独立页面）：
- 基本信息：名称、图标、描述
- AI 配置面板：角色定义（textarea）、对话策略（textarea）、工具策略（textarea）、定稿提示（textarea）、产出物生成模板（textarea，带 `{discussion_summary}` 变量提示）
- 阶段配置：可拖拽排序的阶段列表，每项含 key + label
- UI 文案配置：key-value 表单
- 预览面板：实时预览最终 system prompt 组装效果
...
named_parts = [("角色定义", role_text)]
if strategy_text:
    named_parts.append(("对话策略", strategy_text))
# ... 项目树、目录、文件等动态部分不变
if finalization_text:
    named_parts.append(("定稿提示", finalization_text))
if extra_context:
    named_parts.append(("项目上下文", extra_context))
if tool_strategy_text:
    named_parts.append(("工具使用策略", tool_strategy_text))
```

`build_plan_generation_prompt()` 改造：
